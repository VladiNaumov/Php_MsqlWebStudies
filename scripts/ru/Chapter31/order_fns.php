...
function process_card($card_details) {
  // Подключиться к платежной системе или
  // использовать gpg для шифрования и передать по почте или
  // при желании сохранить сведения в базе данных.

  return true;
}

function insert_order($order_details) {
  // Извлечь order_details как переменные.
  extract($order_details);

  // Установить адрес доставки таким же, как почтовый адрес.
  ...
  // Мы хотим вставить заказ в виде транзакции.
  // Начать транзакцию, отключив автоматическую фиксацию.
  $conn->autocommit(FALSE);

  // Вставить адрес пользователя.
  ...
  $query = "select orderid from orders where
           ...
           "' and amount > (" . (float)$_SESSION['total_price'] .
           "-.001) and amount < (" . (float)$_SESSION['total_price'] .
           "+.001) and date = '" . $conn->real_escape_string($date) .
           "' and order_status = 'PARTIAL' and ship_name = '" .
           $conn->real_escape_string($ship_name) . "' and ship_address = '" .
           $conn->real_escape_string($ship_address) . "' and ship_city = '" .
           $conn->real_escape_string($ship_city) . "' and ship_state = '" .
           $conn->real_escape_string($ship_state) . "' and ship_zip = '" .
           $conn->real_escape_string($ship_zip) . "' and ship_country = '" .
           $conn->real_escape_string($ship_country) . "'";
